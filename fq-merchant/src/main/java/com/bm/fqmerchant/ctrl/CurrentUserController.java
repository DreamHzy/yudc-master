...
public class CurrentUserController extends CommonCtrl{
	...
	@Autowired private IBSysEntitlementService sysEntitlementService;
	@Autowired private IBSysUserService sysUserService;
	@Autowired private IBSysUserAuthService sysUserAuthService;
	...
	public ApiRes currentUserInfo() {
		...
		if(!entIdList.isEmpty()){
			allMenuList = sysEntitlementService.list(BSysEntitlement.gw()
					.in(BSysEntitlement::getEntId, entIdList)
					.in(BSysEntitlement::getEntType, Arrays.asList(CS.ENT_TYPE.MENU_LEFT, CS.ENT_TYPE.MENU_OTHER))
					.eq(BSysEntitlement::getSysType, CS.SYS_TYPE.MCH)
					.eq(BSysEntitlement::getState, CS.PUB_USABLE));
		}
	...
	public ApiRes modifyCurrentUserInfo() {
        ...
        BSysUser updateRecord = new BSysUser();
